...
<script type="text/babel">
    ...
    function App() {
        ...
        const stats = useMemo(() => {
            ...
            processedData.forEach(d => { if (s[d.Category]) { s[d.Category].count++; s[d.Category].totalJJG += d.JJG; s[d.Category].totalHK += d.HK; } });
            Object.keys(s).forEach(k => { s[k].avgJJGperHK = s[k].totalHK > 0 ? s[k].totalJJG / s[k].totalHK : 0; });
            return s;
        }, [processedData]);

        const uniqueAFDs = useMemo(() => { const list = [...new Set(processedData.map(d => d.AFD))].filter(Boolean).sort(); return ['All', ...list]; }, [processedData]);
        ...
        const filteredList = useMemo(() => {
            return processedData.filter(d => { return (filterCategory === 'All' || d.Category === filterCategory) && (filterAFD === 'All' || d.AFD === filterAFD) && d.NAMA.toLowerCase().includes(searchTerm.toLowerCase()); });
        }, [processedData, filterCategory, filterAFD, searchTerm]);
        ...
        const maxHK = useMemo(() => Math.max(...processedData.map(d => d.HK), 30) + 2, [processedData]);
        const maxProd = useMemo(() => Math.max(...processedData.map(d => d.Kg_per_HK), 1000) + 100, [processedData]);
                ...
                <div ref={dashboardRef} className="max-w-7xl mx-auto px-3 sm:px-6 pt-6 w-full">
                    <div className="grid grid-cols-2 lg:grid-cols-5 gap-2 sm:gap-3 mb-6 w-full">
                        <div className="col-span-2 lg:col-span-1 min-w-0"> 
                             <StatCard title="Total Karyawan" value={processedData.length} subtext="Total Populasi" color="gray" icon={Icons.Users} isActive={filterCategory === 'All'} onClick={() => setFilterCategory('All')} />
                        </div>
                    ...
                    <div className="bg-slate-800 rounded-2xl border border-slate-700 shadow-xl shadow-black/20 overflow-hidden w-full min-w-0">
                        <div className="p-4 sm:p-5 border-b border-slate-700 flex flex-col sm:flex-row justify-between items-center gap-4 bg-slate-800">
                            <div className="flex items-center gap-2 w-full sm:w-auto">
                                ...
                                <h3 className="font-bold text-slate-200 truncate">Detail Data Karyawan</h3>
                            </div>
                            <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                                ...
                                <div className="relative w-full sm:w-72 group">
                                    <input type="text" placeholder="Cari nama karyawan..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 text-sm bg-slate-900/50 border border-slate-600 rounded-xl focus:bg-slate-900 focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all outline-none text-slate-200 placeholder-slate-500" />
                            ...
                            <table className="w-full text-left border-collapse table-fixed">
                                ...
                                <tbody className="divide-y divide-slate-700/50 text-[9px] xs:text-[10px] sm:text-sm">
                                    {filteredList.map((row) => (
                                        <tr key={row.ID} className="hover:bg-slate-700/40 transition-colors group">
                                            <td className="px-1 py-2 sm:px-6 sm:py-3 font-medium text-slate-400 group-hover:text-slate-300 transition-colors text-center">{row.AFD}</td>
                        ...
                        <div className="p-4 bg-slate-900/30 border-t border-slate-700 text-xs text-slate-500 flex justify-between font-medium">
                            <span>Total: {processedData.length} data</span>
